...
To enable email capabilities, set the following environment variables. This project uses Amazon Simple Email Service (SES).

**Important**: The template includes a welcome email, which will be enabled once you set your AWS SES credentials. You should customize the email to fit your brand and content or remove it **BEFORE** you enable the email with environment variables. See below.

- `PRIVATE_AWS_REGION`: The AWS region where your SES service is configured (e.g., `us-east-1`).
- `PRIVATE_AWS_ACCESS_KEY_ID`: The Access Key ID for an IAM user with `ses:SendEmail` permissions.
- `PRIVATE_AWS_SECRET_ACCESS_KEY`: The Secret Access Key for the same IAM user.
- `PRIVATE_ADMIN_EMAIL`: The email address to which admin emails will be sent. If unset, admin emails will not be sent.
- `PRIVATE_FROM_ADMIN_EMAIL`: The email address to use as the 'from' address for admin emails. You must verify this address or its domain in the AWS SES console. This defaults to `PRIVATE_ADMIN_EMAIL` if not set.

Before you can send emails, you must configure and verify your sending identity (domain or email address) in the AWS SES console and request production access to move out of the sandbox environment.
